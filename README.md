...
Two options to be explored next. In visualizing the data, we can clearly see that nest attendance increases during rain events. This relationship is nonlinear, so we can approximate the relationship using nonlinear regression. Or we can use belief nets to account for interacting features.
